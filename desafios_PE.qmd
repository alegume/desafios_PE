...
```{r}
pa_ta <- (pa * pta_a) / pta   # P(A|"A")
pb_ta <- (pb * pta_b) / pta   # P(B|"A")
pa_tb <- (pa * ptb_a) / ptb   # P(A|"B")
pb_tb <- (pb * ptb_b) / ptb   # P(B|"B")
cat(pa_ta, pb_ta, pa_tb, pb_tb)
```

Com essas informações, é possível calcular as demais informações necessárias para construir a árvore de decisão a seguir.

```{r}
# Valor esperado após o teste indicar "A" 
v_ta_m <- pa_ta * v_am + pb_ta * v_bm  
v_ta_nm <- pa_ta * v_anm + pb_ta * v_bnm
VA <- max(v_ta_m, v_ta_nm)

# Valor esperado após o teste indicar "B" 
v_tb_m <- pa_tb * v_am + pb_tb * v_bm     #
v_tb_nm <- pa_tb * v_anm + pb_tb * v_bnm  #
VB <- max(v_tb_m, v_tb_nm)
...
arvore <- ToDiagrammeRGraph(dados, direction = "climb") %>%
  set_global_graph_attrs("layout", "dot", "graph") %>%
  add_global_graph_attrs("rankdir", "LR","graph")
render_graph(arvore, width = 500, height = 700)
```
:::
